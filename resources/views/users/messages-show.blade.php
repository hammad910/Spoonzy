...
@section('css')
    <script type="text/javascript">
        var subscribed_active =
            {{ $subscribedToYourContent || $subscribedToMyContent || auth()->user()->isSuperAdmin() || $user->isSuperAdmin() ? 'true' : 'false' }};
    ...
    <section class="section section-sm pb-0 h-100 section-msg position-fixed">
        <div class="d-flex h-100 w-100">
            ...
            <div class="d-none d-lg-block"
                style="height: 100vh; border-right: 1px solid #ddd; padding: 20px; width: 12%; flex-shrink: 0;">
                ...
                <div class="col-md-9 h-100 p-0 first">
                    <div class="flex-grow-1 h-100">
                        <div class="card w-100 rounded-0 h-100 border-top-0">
                            <div class="card-header bg-white pt-4">
                                <div class="media">
                                    ...
                                    <a href="{{ url($user->username) }}" class="mr-3">
                                        <span
                                            class="position-relative user-status @if ($user->active_status_online == 'yes') @if (Helper::isOnline($user->id)) user-online @else user-offline @endif @endif d-block">
                                    ...
                                    <div class="media-body">
                                        ...
                                        @if ($user->active_status_online == 'yes')
                                            ...
                                            @if ($user->hide_last_seen == 'no')
                                                ...
                                                <span id="timeAgo">
                                                    <small
                                                        class="timeAgo @if (Helper::isOnline($user->id)) display-none @endif"
                                                        id="lastSeen"
                                    ...
                                    @if (auth()->user()->verified_id == 'yes' &&
                                        ...
                                        <a href="javascript:void(0);"
                                            class="f-size-20 text-muted float-right mr-3 text-decoration-none @if (Helper::isOnline($user->id)) startAudioCall @else buttonDisabled @endif"
                                            @if (Helper::isOnline($user->id)) data-toggle="tooltip" data-placement="bottom" title="{{ __('general.new_audio_call') }}" @endif
                                            role="button">
                                    ...
                                    @if (auth()->user()->verified_id == 'yes' &&
                                            ...
                                            !auth()->user()->isRestricted($user->id))
                                        <a href="javascript:void(0);"
                                            class="f-size-20 text-muted float-right mr-3 text-decoration-none @if (Helper::isOnline($user->id)) startVideoCall @else buttonDisabled @endif"
                                            @if (Helper::isOnline($user->id)) data-toggle="tooltip" data-placement="bottom" title="{{ __('general.new_video_call') }}" @endif
                                            role="button">
                                            <i class="feather icon-video"></i>
                                        </a>
                                    @endif

                                    <a href="javascript:void(0);" class="f-size-20 text-muted float-right"
                                        id="dropdown_options" role="button" data-toggle="dropdown" aria-haspopup="true"
                                        aria-expanded="true">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdown_options">
                                        ...
                                        @if (
                                            ...
                                            <button type="button" class="dropdown-item requestLivePrivateModal"
                                                data-toggle="tooltip" data-placement="bottom">
                                                <i class="feather icon-video mr-2"></i>
                                                {{ __('general.request_private_live_stream') }}
                                            </button>
                                        @endif

                                        @if ($messages->count() != 0 && $settings->users_can_delete_messages)
                                            <form method="POST" action="{{ url('conversation/delete', $user->id) }}"
                                                class="d-inline">
                                                @csrf
                                        ...
                                        @if (auth()->user()->isRestricted($user->id))
                                            <button type="button" class="dropdown-item removeRestriction"
                                                data-user="{{ $user->id }}" id="restrictUser">
                                                <i class="fas fa-ban mr-2"></i> {{ __('general.remove_restriction') }}
                                            </button>
                                        @else
                                            <button type="button" class="dropdown-item" data-user="{{ $user->id }}"
                                                id="restrictUser">
                            ...
                            <div class="content px-4 py-3 custom-scrollbar container-msg" id="contentDIV"
                                data="{{ $user->id }}">
                            ...
                            @if ((!auth()->user()->checkRestriction($user->id) && $user->allow_dm) || auth()->user()->isSuperAdmin())
                                <div class="card-footer bg-white position-relative">
                                    ...
                                    @if ($subscribedToYourContent || $subscribedToMyContent || auth()->user()->isSuperAdmin() || $user->isSuperAdmin())
                                        ...
                                        <form action="{{ url('message/send') }}" method="post" accept-charset="UTF-8"
                                            id="formSendMsg" enctype="multipart/form-data">
                                            <input type="hidden" name="id_user" id="id_user"
                                                value="{{ $user->id }}">
                                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                            <input type="file" name="zip" id="zipFile"
                                                accept="application/x-zip-compressed" class="visibility-hidden">
                                            ...
                                            <div class="w-100 mr-2 position-relative">
                                                ...
                                                <textarea class="form-control textareaAutoSize emojiArea border-0" data-post-length="{{ $settings->update_length }}"
                                                    rows="1" placeholder="{{ __('general.write_something') }}" id="message" name="message"></textarea>
                                            </div>
                                            ...
                                            <div class="form-group display-none mt-2" id="price">
                                                <div class="input-group mb-2">
                                                    ...
                                                    <input class="form-control isNumber" autocomplete="off"
                                                        name="price" placeholder="{{ __('general.price') }}"
                                                        type="text">
                                                </div>
                                            </div><!-- End form-group -->
                                            ...
                                            <div class="w-100 mb-2">
                                                ...
                                                <a href="javascript:void(0)" id="removePhoto"
                                                    class="text-danger p-1 small display-none btn-tooltip"
                                            ...
                                            <input type="file" name="media[]" id="file"
                                                ...
                                                class="visibility-hidden filepond input-fileuploader">

                                            <div class="justify-content-between mt-3 align-items-center">

                                                <button type="button"
                                                    class="btnMultipleUpload btn btn-upload btn-tooltip e-none align-bottom @if (auth()->user()->dark_mode == 'off') text-primary @else text-white @endif rounded-pill"
                                                    data-toggle="tooltip" data-placement="top"
                                                    title="{{ __('general.upload_media') }} ({{ $settings->disable_audio ? __('general.photo_video') : __('general.media_type_upload') }})">
                                                ...
                                                @if (auth()->user()->verified_id == 'yes' && $settings->allow_vault)
                                                    <button type="button"
                                                        class="btn btn-upload btn-tooltip btnShowVault e-none align-bottom @if (auth()->user()->dark_mode == 'off') text-primary @else text-white @endif rounded-pill"
                                                        data-toggle="tooltip" data-placement="top"
                                                        title="{{ __('general.add_media_from_vault') }}">
                                                ...
                                                @if (
                                                    (auth()->user()->verified_id == 'yes' &&
                                                        ...
                                                        (!$settings->ppv_only_free_accounts && auth()->user()->verified_id == 'yes'))
                                                    <button type="button" id="setPrice"
                                                        class="btn btn-upload btn-tooltip e-none align-bottom @if (auth()->user()->dark_mode == 'off') text-primary @else text-white @endif rounded-pill"
                                                        data-toggle="tooltip" data-placement="top"
                                                        title="{{ __('general.set_price_for_msg') }}">
                                                        <i class="feather icon-tag align-middle"
                                                ...
                                                @if ($user->verified_id == 'yes' && $settings->disable_tips == 'off')
                                                    <button type="button"
                                                        class="btn btn-upload btn-tooltip e-none align-bottom @if (auth()->user()->dark_mode == 'off') text-primary @else text-white @endif rounded-pill"
                                                        data-toggle="modal" title="{{ __('general.tip') }}"
                                                        data-target="#tipForm"
                                                ...
                                                <div
                                                    class="d-inline-block float-right rounded-pill mt-1 position-relative">
                                                    <div class="btn-blocked display-none"></div>
                                                    <button type="submit" id="button-reply-msg" disabled
                                                        data-send="{{ __('auth.send') }}"
                                                        data-wait="{{ __('general.send_wait') }}"
                                    ...
                                    @else
                                        <div class="alert alert-primary m-0 alert-dismissible fade show" role="alert">
                                            ...
                                            @php
                                                $nameUser =
                                                    $user->hide_name == 'yes' ? $user->username : $user->first_name;
                                            @endphp
                                            {!! __('general.show_form_msg_error_subscription_', [
                                                'user' => '<a href="' . url($user->username) . '" class="link-border text-white">' . $nameUser . '</a>',
                                            ]) !!}
                                        </div>
                                    @endif
