...
125px @endif 0; background: #505050 @if ($user->cover != '') url('{{ Helper::getFile(config('path.cover') . $user->cover) }}') no-repeat center center; background-size: cover; @endif">
                    ...
                    <div class="col-lg-12">
                        ...
                        @if ($user->verified_id == 'yes')
                            <ul class="nav nav-profile justify-content-center nav-fill">
                                ...
                                @if ($totalReels != 0)
                                    <li
                                        class="nav-link @if (request()->path() == $user->username . '/reels') active @endif navbar-user-mobile">
                                ...
                                @if (!$settings->disable_audio)
                                    <li
                                        ...
                                        <a href="{{ request()->path() == $user->username . '/audio' ? 'javascript:;' : url($user->username, 'audio') }}"
                                            title="{{ __('general.audio') }}"><i class="feather icon-mic"></i> <span
                                                class="d-lg-inline-block d-none">{{ __('general.audio') }}</span></a>
                                    </li>
                                @endif

                                @if ($settings->shop || (!$settings->shop && $userProducts->count() != 0))
                                    <li
                                        class="nav-link @if (request()->path() == $user->username . '/shop') active @endif navbar-user-mobile">
                                        <small
                                        ...
                                        <a href="{{ request()->path() == $user->username . '/shop' ? 'javascript:;' : url($user->username, 'shop') }}"
                                            ...
                                            <span class="d-lg-inline-block d-none">{{ __('general.shop') }}</span></a>
                                    </li>
                                @endif
                                ...
                                @if ($totalEpub != 0)
                                    <li
                                        class="nav-link @if (request()->path() == $user->username . '/epub') active @endif navbar-user-mobile">
                                        ...
                                        <a href="{{ request()->path() == $user->username . '/epub' ? 'javascript:;' : url($user->username, 'epub') }}"
                                            ...
                                            <span class="d-lg-inline-block d-none">{{ __('general.epub') }}</span></a>
                                    </li>
                                @endif
                        ...
                        @if ($reels->count() > 5)
                            <div>
                                <select class="ml-2 custom-select mb-2 mb-lg-0 w-auto" id="filter">
                                    <option @if (!request()->get('sort')) selected @endif
                                        value="{{ url($user->username) . '/reels' }}">{{ __('general.latest') }}
                                    </option>
                                    <option @if (request()->get('sort') == 'oldest') selected @endif
                                        value="{{ url($user->username) . '/reels?sort=oldest' }}">
                                        {{ __('general.oldest') }}</option>
                                    <option @if (request()->get('sort') == 'popular') selected @endif
                                        value="{{ url($user->username) . '/reels?sort=popular' }}">
                                        {{ __('general.popular') }}</option>
                                    <option @if (request()->get('sort') == 'views') selected @endif
                                        value="{{ url($user->username) . '/reels?sort=views' }}">
                                        {{ __('general.views') }}</option>
                                </select>
                            </div>
                        @endif
                    </div>
                        ...
                        <div class="container">
                            ...
                            @else
                                <div class="my-5 text-center no-updates w-100">
                                    <span class="btn-block mb-3">
                                        <svg xmlns="http://www.w3.org/2000/svg"
                                            class="align-bottom border-0 ico-no-result" fill="currentColor"
                    ...
                    @if ($userProducts->count() != 0)
                        <div
                            ...
                            <div>
                                <select class="ml-2 custom-select mb-2 mb-lg-0 w-auto" id="filter">
                                    <option @if (!request()->get('sort')) selected @endif
                                        value="{{ url($user->username) . '/shop' }}">{{ __('general.latest') }}
                                    </option>
                                    <option @if (request()->get('sort') == 'oldest') selected @endif
                                        value="{{ url($user->username) . '/shop?sort=oldest' }}">
                                        {{ __('general.oldest') }}</option>
            ...
            @if (auth()->check() && auth()->id() != $user->id)
                <div class="modal fade modalReport" id="reportCreator" tabindex="-1" role="dialog"
                    aria-hidden="true">
                    <div class="modal-dialog modal-danger modal-sm">
                        <div class="modal-content">
                            ...
                            <!-- form start -->
                            <form method="POST" action="{{ url('report/creator', $user->id) }}"
                                enctype="multipart/form-data">
                                <div class="modal-body">
                                    @csrf
                                    <!-- Start Form Group -->
                                    <div class="form-group">
                                        ...
                                        <select name="reason" class="form-control custom-select">
                                            ...
                                            <option value="violent_sexual">{{ __('admin.violent_sexual_content') }}
                                            </option>
